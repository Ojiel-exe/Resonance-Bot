...
module.exports = {
    name: "invitelogs",
    aliases: ["invitelogs"],
    description: "Setup Invite Create Logs",
    category: "logs",
    // timeout: 5000,
    usage: "!invitelogs",
    ...
    run: async(client, message, args) => {
        ...
        const choice = new MessageEmbed()
            ...
            **Welcome To \`Leave Logger Event\` Command!**
            
            **Please Choose The Option Below**

            <a:HC_arrow1:880710912376459285> ** Type : \`${p}invitelogs set <id | mention channel>\` **
            <a:HC_arrow1:880710912376459285> ** Type : \`${p}invitelogs check\` **
            <a:HC_arrow1:880710912376459285> ** Type : \`${p}invitelogs remove\` **
            `)
        ...
        if (args[0] === 'set') {
            ...
            message.reply(`Invite Create Logs Set To <#${targetChannel.id}>!`)
        }
        if (args[0] === 'check') {
            const { guild } = message
            await invitelogs.findOne({
                _id: guild.id
            }, async(err, data) => {
                if (!data) return message.reply('This Guild Has No Data To Stored')
                const channel = client.channels.cache.get(data.Channel);

                message.reply(`Channel Leave Logs | ${channel}`);
            })
        }
        if (args[0] === 'remove') {
            const { guild } = message
