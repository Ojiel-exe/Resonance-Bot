...
module.exports = {
    ...
    run: async(client, message, args) => {
        ...
        const choice = new MessageEmbed()
            ...
            **Welcome To \`Level Logger Event\` Command!**
            
            **Please Choose The Option Below**

            <a:HC_arrow1:880710912376459285> ** Type : \`${p}levellogs set <id | mention channel>\` **
            <a:HC_arrow1:880710912376459285> ** Type : \`${p}levellogs check\` **
            <a:HC_arrow1:880710912376459285> ** Type : \`${p}levellogs remove\` **
            `)
        ...
        if (args[0] === 'set') {
            ...
            await levellogs.findOneAndUpdate({
                _id: guild.id,
            }, {
                _id: guild.id,
            ...
            message.reply(`Logs Level Channel Set To <#${targetChannel.id}>!`)
        }
        if (args[0] === 'check') {
            const { guild } = message
            await levellogs.findOne({
                _id: guild.id
            }, async(err, data) => {
                if (!data) return message.reply('This Guild Has No Data To Stored')
                const channel = client.channels.cache.get(data.Channel);

                message.reply(`Channel Level Logs | ${channel}`);
            })
        }
        if (args[0] === 'remove') {
            const { guild } = message
