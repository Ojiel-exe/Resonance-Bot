...
module.exports = {
    ...
    run: async(client, message, args) => {
       ...
       const choice = new MessageEmbed()
            ...
            **Welcome To \`Message Delete Logger Event\` Command!**
            
            **Please Choose The Option Below**

            <a:HC_arrow1:880710912376459285> ** Type : \`${p}messagedeletes set <id | mention channel>\` **
            <a:HC_arrow1:880710912376459285> ** Type : \`${p}messagedeletes check\` **
            <a:HC_arrow1:880710912376459285> ** Type : \`${p}messagedeletes remove\` **
            `)
        ...
        if (args[0] === 'set') {
            ...
            await messagedeletes.findOneAndUpdate({
                _id: guild.id,
            }, {
                _id: guild.id,
            ...
            message.reply(`Logs Message Delete Channel Set To <#${targetChannel.id}>!`)
        }
        if (args[0] === 'check') {
            const { guild } = message
            await messagedeletes.findOne({
                ...
                message.reply(`Message Delete Logs | ${channel}`);
            })
        }
        if (args[0] === 'remove') {
            const { guild } = message
