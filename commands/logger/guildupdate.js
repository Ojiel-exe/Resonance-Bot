...
module.exports = {
    name: "guildupdates",
    aliases: ["guildupdate", "gu"],
    description: "Setup Guild Update Logs",
    category: "logs",
    // timeout: 5000,
    usage: "!guildupdate",
    ...
    run: async(client, message, args) => {
        ...
        // message.reply(`**This Commands Undermaintenance, Please Wait Until The Developer Fix It!**`)
         const choice = new MessageEmbed()
            .setColor("DARK-BLUE")
            .setDescription(stripIndents `
            **Welcome To \`Guild Update Logger Event\` Command!**
            
            **Please Choose The Option Below**

            <a:HC_arrow1:880710912376459285> ** Type : \`${p}guildupdate set <id | mention channel>\` **
            <a:HC_arrow1:880710912376459285> ** Type : \`${p}guildupdate check\` **
            <a:HC_arrow1:880710912376459285> ** Type : \`${p}guildupdate remove\` **
            `)
        ...
        if (args[0] === 'set') {
            ...
            await guildupdates.findOneAndUpdate({
                _id: guild.id,
            }, {
                _id: guild.id,
            ...
            message.reply(`Guild Update Channel Set To <#${targetChannel.id}>!`)
        }
        if (args[0] === 'check') {
            const { guild } = message
            await guildupdates.findOne({
                _id: guild.id
            }, async(err, data) => {
                if (!data) return message.reply('This Guild Has No Data To Stored')
                const channel = client.channels.cache.get(data.Channels);

                message.reply(`Channel Guild Update | ${channel}`);
            })
        }
        if (args[0] === 'remove') {
            const { guild } = message
