...
module.exports = {
    name: "channeldelete",
    aliases: ["channeldeletelogs"],
    description: "Setup Channel Delete Logs",
    category: "logs",
    // timeout: 5000,
    usage: "!channeldelete",
    ...
    run: async(client, message, args) => {
        ...
        const choice = new MessageEmbed()
            ...
            **Welcome To \`Channel Delete Logger Event\` Command!**
        ...
        if (args[0] === 'set') {
            ...
            await channeldelete.findOneAndUpdate({
                _id: guild.id,
            }, {
                _id: guild.id,
            ...
            message.reply(`Logs Channel Delete Set To <#${targetChannel.id}>!`)
        }
        if (args[0] === 'check') {
            const { guild } = message
            await channeldelete.findOne({
                _id: guild.id
            }, async(err, data) => {
